    ...
    try{
        ...
        for(let key of allowedFields){
            if(req.body[key] !== undefined){
                const dbField = key === "workMode" ? "work_mode" : key === "appliedAt" ? "applied_at" : key;
...
export const deleteApplication = async (req, res) => {
    ...
    try{
        ...
        await pool.query("DELETE FROM applications WHERE user_id=$1 AND id=$2", [userId, applicationId]);
...
export const getUserApplications = async (req, res) => {
    ...
    try{
        ...
        const result = await pool.query("SELECT * FROM applications WHERE user_id=$1 ORDER BY applied_at DESC", [userId]);
